...
%% API
%%====================================================================
decode(<<0:2, Type:14, Len:16, Magic:32, TrID:96, Body:Len/binary, Tail/binary>> = Data,
       Transport) ->
...
enc_attrs(Msg) ->
    iolist_to_binary([enc_attr(?STUN_ATTR_SOFTWARE, Msg#stun.'SOFTWARE'),
                      ...
                      enc_xor_peer_addr(Msg#stun.magic, Msg#stun.trid, Msg#stun.'XOR-PEER-ADDRESS'),
...
dec_addr(1, <<A1, A2, A3, A4>>) ->
    {A1, A2, A3, A4};
dec_addr(2, <<A1:16, A2:16, A3:16, A4:16, A5:16, A6:16, A7:16, A8:16>>) ->
...
enc_xor_addr(Type, Magic, TrID, {{A1, A2, A3, A4, A5, A6, A7, A8}, Port}) ->
    XPort = Port bxor (Magic bsr 16),
    <<Addr:128>> = <<A1:16, A2:16, A3:16, A4:16, A5:16, A6:16, A7:16, A8:16>>,
    XAddr = Addr bxor (Magic bsl 96 bor TrID),
    enc_attr(Type, <<0, 2, XPort:16, XAddr:128>>).

enc_xor_peer_addr(Magic, TrID, AddrPortList) ->
    [enc_xor_addr(?STUN_ATTR_XOR_PEER_ADDRESS, Magic, TrID, AddrPort)

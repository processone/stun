...
%% API
-export([start_link/2, start/2, stop/1, socket_type/0, tcp_init/2, udp_init/2,
         udp_recv/5]).
%% gen_fsm callbacks
        ...
        {ok, Addr} ->
            ...
            case maybe_starttls(Sock, SockMod, CertFile) of
                {ok, NewSock} ->
                    ...
                    {ok, session_established, State#state{tref = TRef, sock = NewSock}};
                {error, Why} ->
                    {stop, Why}
            end;
        Err ->
...
handle_info({tcp, _Sock, TLSData}, StateName, #state{sock_mod = fast_tls} = State) ->
        ...
        _ ->
            ...
            case turn:start(Opts) of
                ...
                {error, limit} ->
                    ...
                    R = Resp#stun{class = error, 'ERROR-CODE' = stun_codec:error(486)},
                    send(State, R, Secret);
                {error, stale} ->
                ...
                Err ->
                    ...
                    R = Resp#stun{class = error, 'ERROR-CODE' = stun_codec:error(500)},
                    send(State, R, Secret)
            end
    end;
...
prepare_state(Opts, Sock, Peer, SockMod) when is_list(Opts) ->
    ...
    case proplists:get_bool(use_turn, Opts) of
        true ->
            lists:foldl(fun ({turn_ip, IP}, State) ->
                            ...
                            ({turn_ipv4_address, IP}, State) ->
                                case prepare_addr(IP) of
                                    {ok, Addr} ->
                                        State#state{relay_ipv4_ip = Addr};
                                    {error, _} ->
                                        ?LOG_ERROR("Wrong 'turn_ipv4_address' value: ~p", [IP]),
                                        State
                                end;
                            ...
                            ({turn_max_allocations, Wrong}, State) ->
                                ?LOG_ERROR("Wrong 'turn_max_allocations' value: ~p", [Wrong]),
                                State;
                            ({turn_max_permissions, N}, State)
                                when is_integer(N) andalso N > 0 orelse is_atom(N) ->
                                State#state{max_permissions = N};
                            ({turn_max_permissions, Wrong}, State) ->
                                ?LOG_ERROR("Wrong 'turn_max_permissions' value: ~p", [Wrong]),
                                State;
                            ({turn_blacklist, B}, State) ->
                                case lists:all(fun is_valid_subnet/1, B) of
                                    ...
                                    false ->
                                        ?LOG_ERROR("Wrong 'turn_blacklist' value: ~p", [B]),
                                        State
                                end;
                            ({shaper, S}, State)
...
now_priority() ->
    {p1_time_compat:monotonic_time(micro_seconds),
     p1_time_compat:unique_integer([monotonic])}.
...
run_hook(HookName,
         #state{session_id = ID,
                ...
                sock_mod = SockMod,
                hook_fun = HookFun},
         #stun{'USERNAME' = User, 'REALM' = Realm} = Msg)

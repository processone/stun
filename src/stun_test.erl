...
add_udp_listener_test() ->
    ?assertEqual(ok,
                 stun_listener:add_listener(?STUN_IP,
                                            ?STUN_PORT,
                                            udp,
                                            [use_turn,
                                             {auth_type, user},
...
bind_udp_test() ->
    ...
    ?assertMatch({ok, #stun{trid = TrID, 'XOR-MAPPED-ADDRESS' = Addr}},
...
bind_tcp_test() ->
    ...
    Msg = #stun{method = ?STUN_METHOD_BINDING,
                class = request,
                trid = TrID},
    {ok, Socket} = gen_tcp:connect(?STUN_IP, ?STUN_PORT, [binary, {active, false}]),
    {ok, Addr} = inet:sockname(Socket),
...
bind_tls_test() ->
    ...
    Msg = #stun{method = ?STUN_METHOD_BINDING,
                class = request,
                trid = TrID},
    {ok, Socket} = gen_tcp:connect(?STUN_IP, ?STUNS_PORT, [binary, {active, true}]),
    {ok, TLSSocket} = fast_tls:tcp_to_tls(Socket, [{certfile, <<"certfile.pem">>}, connect]),
...
allocate_udp_test() ->
    ...
    TrID1 = mk_trid(),
    Msg1 =
        #stun{method = ?STUN_METHOD_ALLOCATE,
              class = request,
              trid = TrID1},
    ...
    {ok,
     #stun{trid = TrID1,
           ...
           'NONCE' = Nonce,
           'REALM' = ?REALM}} =
        stun_codec:decode(PktIn1, datagram),
    ...
    {ok,
     #stun{trid = TrID2,
        ...
        stun_codec:decode(PktIn2, datagram),
    %% Creating permission for the peer
    TrID3 = mk_trid(),
    Msg3 =
        #stun{method = ?STUN_METHOD_CREATE_PERMISSION,
              trid = TrID3,
    ...
    {ok, #stun{trid = TrID3, class = response}} = stun_codec:decode(PktIn3, datagram),
    %% Sending some data to the peer. Peer receives it.
    Data1 = crypto:strong_rand_bytes(20),
    TrID4 = mk_trid(),
    Msg4 =
        #stun{method = ?STUN_METHOD_SEND,
              trid = TrID4,
              class = indication,
              'XOR-PEER-ADDRESS' = [PeerAddr],
              'DATA' = Data1},
    ...
    TrID5 = mk_trid(),
    Msg5 =
        #stun{method = ?STUN_METHOD_CHANNEL_BIND,
              trid = TrID5,
              class = request,
    ...
    ?assertMatch({ok, #stun{trid = TrID5, class = response}},
...
bind_udp(Host, Port) ->
    TrID = mk_trid(),
    MsgOut =
        #stun{method = ?STUN_METHOD_BINDING,
              class = request,
              trid = TrID},
    try
        ...
        {ok, {_, _, PktIn}} = gen_udp:recv(Socket, 0, ?RECV_TIMEOUT),
        {ok, MsgIn = #stun{trid = TrID, 'XOR-MAPPED-ADDRESS' = _Addr}} =
            stun_codec:decode(PktIn, datagram),
